...
import { applicationSources } from "../lib/application-options";
...
export default function ApplicationDetailsModal({ dateLocale, application, onClose, onSaved }: Props) {
  ...
  return <div className="modal-backdrop" onClick={onClose}>
    <section className="details-modal" role="dialog" aria-modal="true" aria-labelledby="details-title" onClick={event => event.stopPropagation()}>
      ...
      {!editing ? <div className="details-body">
        ...
        <div className="details-grid">{metadata.map(([label, item]) => <div key={label}><span>{label}</span><strong>{label === "Applied" || label === "Next action" ? formatStoredDate(String(item), dateLocale) : item}</strong></div>)}</div>
